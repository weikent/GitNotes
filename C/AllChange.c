...
int len = 0;
int trave_dir(char* path, char *pdfCommand)
{
    DIR *d; 
    struct dirent *file; 
    struct stat buf;    
    ...
    while((file = readdir(d)) != NULL)
    {
        ...
        if(stat(file->d_name, &buf) >= 0 && !S_ISDIR(buf.st_mode) )
        {
            ...
            if (strstr(file->d_name, ".html"))
            {
                ...
                sprintf(command, "%s %s %s.pdf",pdfCommand, file->d_name, name);
                printf ("command = %s\n", command);
                system(command);
                remove(file->d_name);
                printf ("%d\n", len);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        printf ("maybe you need add a htmltopdf command, like [wkhtmltopdf] \n");
        return 0;
    }

    trave_dir(".", argv[1]);//trave_dir("./ChildrenDir");
    return 0;
}